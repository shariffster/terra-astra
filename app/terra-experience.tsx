...
export default function TerraExperience(){
 ...
 const composeTrigger=useRef<HTMLButtonElement|null>(null),savedStartup=useRef<Composition|null>(null),preferencesReady=useRef(false);
 ...
 const configure=useCallback((partial:Partial<ViewOptions>)=>{setOptions(current=>({...current,...partial}));},[]);
 const applyComposition=useCallback(async(c:Composition)=>{
 ...
 useEffect(()=>{engine.current?.compositionPanel(!!compositionTab);},[compositionTab,ready]);
 useEffect(()=>{if(!ready||genesis.busy||preferencesReady.current)return;preferencesReady.current=true;const c=savedStartup.current;if(c)void applyComposition(c).catch(e=>setError(e instanceof Error?e.message:'Saved settings could not be restored.'));},[ready,genesis.busy,applyComposition]);
 useEffect(()=>{if(!ready||!preferencesReady.current||!world)return;const timer=setTimeout(()=>{try{localStorage.setItem(COMPOSITION_STORAGE,JSON.stringify(composition('Last view',options,world.presentation,world.layers)));}catch{}},350);return()=>clearTimeout(timer);},[options,world,ready]);
 ...
 return <main className={`experience stage-${stage} view-${view}${selected?' has-story':''}${remembered?' has-memory':''}${exploring?' is-exploring':''}${settling?' is-settling':''}${options.motion?'':' motion-paused'}${astra?' in-astra':''}${editingPersonal?' editing-personal':''}${personalReturned?' personal-returned':''}${compositionTab?' composing-earth':''}${genesis.busy?' genesis-active':ready?' is-born':''}`} data-genesis={genesis.phase} data-scale={world?.tier??'planet'} data-target={world?.targetId??''} data-transformation={transformation.phase}>
  ...
  {compositionTab&&ready&&!genesis.busy&&!astra?<CompositionControls tab={compositionTab} setTab={setCompositionTab} onClose={closeComposition} options={options} onChange={configure} state={world} onApply={applyComposition} disabled={!!world?.busy}/>:null}
