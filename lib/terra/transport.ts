...
export const DEFAULT_TRANSPORT:TransportOptions={aircraft:{...base,count:200,mode:'local',ahead:.45,behind:.7},ships:{...base,count:264,travellerLight:1.1},cables:{...base,count:96,pulse:true,pulseRate:.26,pulseDepth:.6,travellerLight:1.45,trailLight:1.25,tail:1.6},satellites:{...base,count:84,mode:'trail',distribution:'even'}};
...
const marineActivityKeys=['count','travellerLight','trailLight','tail','pulse','pulseRate','pulseDepth'] as const;
/** An explicit marine-only preset leaves routes, visibility and other families alone. */
export function livelierSeas(value:TransportOptions):TransportOptions {
 const next=cloneTransport(value);
 for(const family of ['ships','cables'] as const)for(const key of marineActivityKeys)Object.assign(next[family],{[key]:DEFAULT_TRANSPORT[family][key]});
 return next;
}
/** Upgrade only untouched legacy marine families in the automatic last view.
 * Named saves and imports bypass this: they remain exact comparisons. */
export function refreshMarineDefaults(value:TransportOptions):TransportOptions {
 const next=cloneTransport(value),legacy={ships:{...base,count:176},cables:{...base,count:14,pulse:true}};
 for(const f of ['ships','cables'] as const)if((Object.keys(legacy[f]) as (keyof FamilyOptions)[]).every(k=>value[f][k]===legacy[f][k]))next[f]={...DEFAULT_TRANSPORT[f]};
 return next;
}
