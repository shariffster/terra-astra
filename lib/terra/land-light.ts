...
export function structureLandLight(data:Float32Array) {
 const cells=new Map<string,number>();
 const cell=(i:number)=>{const r=Math.hypot(data[i],data[i+1],data[i+2]);return [data[i]/r*32,data[i+1]/r*32,data[i+2]/r*32];};
 for(let i=0;i<data.length;i+=6){const key=cell(i).map(Math.round).join(',');cells.set(key,(cells.get(key)??0)+Math.max(.03,data[i+3]-.2));}
 ...
 for(let i=0;i<data.length;i+=6){const v=cell(i),base=v.map(Math.floor),t=v.map((n,j)=>n-base[j]);let neighbourhood=0;
  ...
  data[i+3]=.012+.96*Math.pow(strength,1.2)*(.20+1.1*cluster);
 }return data;
}
