...
import { compositionFraming, type PanelBounds } from './composition-framing';
...
export type Engine = { audioState:()=>AudioWorldState; beginGenesis:()=>void; replayGenesis:()=>void; skipGenesis:()=>void; command:(command:WorldCommand)=>Promise<WorldCommandResult>; worldState:()=>WorldState; transform:(open:boolean)=>void; personal:(places:PersonalPlaces|null)=>void; descend:()=>Promise<void>; orbit:()=>void; zoom:(factor:number)=>void; rotate:(dx:number,dy:number)=>void; select:(id:string|null)=>void; configure:(o:ViewOptions)=>void; storyInset:(pixels:number)=>void; compositionPanel:(open:boolean,bounds?:PanelBounds)=>void; narrativePanel:(visible:boolean)=>void; compareLight:(light:ViewOptions|null)=>void; view:(view:EarthView)=>void; region:(region:StudyRegion)=>void; dispose:()=>void };
...
export async function createEarth(host:HTMLDivElement,markers:HTMLDivElement,callbacks:Callbacks,signal:AbortSignal,initialLight?:ViewOptions):Promise<Engine>{
 ...
 const smoothCameraOffset=new THREE.Vector2();let compositionOpen=false,narrativeVisible=true;let compositionBounds:PanelBounds|null=null;
 ...
 smoothCameraOffset.lerp(new THREE.Vector2((available?.x??(mobile?(stage==='orbit'&&scaleTier==='planet'&&viewMode==='globe'&&openProgress===0?w*.035:0):-w*desktopInset))*born.settled,(available?.y??offsetY)*born.settled),frameMix);
 const panelZoom=available&&scaleTier==='planet'&&viewMode==='globe'?available.zoom:1;
