...
export async function createEarth(host:HTMLDivElement,markers:HTMLDivElement,callbacks:Callbacks,signal:AbortSignal,initialLight?:ViewOptions):Promise<Engine>{
 ...
 celestial.render(time,dt,{...displayedLight,moon:options.moon,sun:options.sun,nebula:options.nebula,motion:options.motion},born.settled*global*(1-opened)*(1-cut),{x:(celestialCentre.x*.5+.5)*w,y:(-.5*celestialCentre.y+.5)*h,r:skyEarthRadius},compositionOpen?compositionBounds:null,stillFrames<3||!options.motion);
