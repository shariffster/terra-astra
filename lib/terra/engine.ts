...
import { WORLD_TARGETS, validateWorldCommand, type GenesisState, type WorldState, type WorldCommand, type WorldCommandResult, type ScaleTier, type WorldLayer } from '../world/commands';
...
export async function createEarth(host:HTMLDivElement,markers:HTMLDivElement,callbacks:Callbacks,signal:AbortSignal):Promise<Engine>{
 ...
 function updateSignals(nowSeconds:number,visibility:number){for(const v of signalViews){
  ...
  for(let i=0;i<v.records.length;i++){
   ...
   for(let k=0;k<v.trailCount;k++){
    ...
    if(visibility>=.002){sampleSignal(v.records[i],nowSeconds,signalOutput,v.records[i].trailSeconds*fraction*Math.min(1,Math.max(0,age)/AWAKENING[v.layer].tail));v.positions.set(signalOutput,(i*v.trailCount+k)*3);}
   }
  }
  v.cloud.points.geometry.setDrawRange(0,active*v.trailCount);v.brightness.needsUpdate=true;
 ...
 async function executeCommand(cmd:WorldCommand,queuedAt:number,sequence:number,epoch:number):Promise<WorldCommandResult>{
  ...
  if(cmd.type==='focusLayer'){awakening.depart();layerFlags[cmd.layer]=cmd.enabled??!layerFlags[cmd.layer];stillFrames=0;notifyWorld();return {ok:true,command:cmd};}
  const target=cmd.type==='resetView'?undefined:cmd.type==='flyTo'||cmd.type==='highlightTarget'?WORLD_TARGETS.find(t=>t.id===cmd.targetId):WORLD_TARGETS.find(t=>t.id===targetId);
  ...
  try{
   ...
   const destinationAlt=tier==='planet'?homeAltitude():tier==='region'?(trenchHorizon?.62:.20):tier==='city'?(SPECIAL_CITIES[target?.id??'']?SPECIAL_CITIES[target!.id].cityAltitude*Math.max(1,host.clientHeight/host.clientWidth*.82):.0018):(SPECIAL_CITIES[target?.id??'']?SPECIAL_CITIES[target!.id].streetAltitude*Math.max(1,host.clientHeight/host.clientWidth*.82):.00075);
 ...
 function command(input:WorldCommand):Promise<WorldCommandResult>{const cmd=validateWorldCommand(input);if(!cmd)return Promise.resolve({ok:false,command:input,reason:'Invalid world command'});const queuedAt=performance.now(),sequence=++commandSequence,epoch=journeyEpoch;host.dataset.worldCommandQueuedAt=queuedAt.toFixed(1);host.dataset.worldCommandStatus='queued';const result=commandChain.then(()=>executeCommand(cmd,queuedAt,sequence,epoch));commandChain=result.catch(()=>undefined);return result;}
 const pointer=new Map<number,{x:number;y:number}>();let dragged=false,initial={x:0,y:0},pinch=0;
 function rotate(dx:number,dy:number){if(flight||morph||genesisProgress<1)return;callbacks.interact();lastInteraction=performance.now();stillFrames=0;const scale=stage==='city'?targetAlt*2*Math.tan(21*R)/(R*host.clientHeight):.18*Math.min(1,targetAlt/1.4);targetLon-=dx*scale;targetLat=clamp(targetLat+dy*scale,-80,80);if(stage==='city'){const ny=targetId==='new-york',special=SPECIAL_CITIES[targetId??''];const bounds=special?.bounds??(ny?[-74.015,40.707,-73.975,40.737]:[103.810,1.272,103.890,1.330]);targetLon=clamp(targetLon,bounds[0],bounds[2]);targetLat=clamp(targetLat,bounds[1],bounds[3]);}}
 function zoom(f:number){if(flight||morph||genesisProgress<1)return;callbacks.interact();lastInteraction=performance.now();stillFrames=0;targetAlt=clamp(targetAlt*f,stage==='city'?(SPECIAL_CITIES[targetId??'']?.minAltitude??.00055):openProgress>0?openAltitude()*.78:viewMode==='oblique'?.32:scaleTier==='region'?.10:.42,stage==='city'?.004:homeAltitude()*(openProgress>0?2.5:1.8));}
 ...
 function frame(now:number){if(disposed||graphicsLost||document.hidden)return;if(!options.motion&&stillFrames>2&&!flight&&!morph&&genesisProgress>=1&&personalSettledAt===null&&awakening.state!=='awakening'){raf=requestAnimationFrame(frame);return;}if(fallback&&now-lastTime<50){raf=requestAnimationFrame(frame);return;}const elapsed=Math.max(0,(now-lastTime)/1000),dt=Math.min(elapsed,.06);lastTime=now;awakening.advance(elapsed,options.motion);if(options.motion)time+=dt;frameCount++;stillFrames++;
 ...
 return {replayGenesis,skipGenesis,command,worldState,transform:setTransformation,personal:setPersonal,descend:async()=>{if(flight||morph||genesisProgress<1||openProgress>0)return;try{personalSettledAt=null;personalArrivalSent=false;clearStory();remembered=null;remembering=false;settledAt=null;arrivalSent=false;targetId='singapore';scaleTier='city';loadingDetail=true;fly(1.2965,103.851,.0018,'city');notifyWorld();try{await ensureCity();}finally{loadingDetail=false;stillFrames=0;}if(disposed)return;}catch(e){if(!signal.aborted)callbacks.error('Singapore could not load. Your Earth is still here; try entering again.');throw e;}},orbit:()=>{if(flight||morph||stage!=='city')return;clearStory();remembering=!!remembered;settledAt=null;arrivalSent=false;targetId=null;scaleTier='planet';fly(19,95,homeAltitude(),'orbit');notifyWorld();},zoom,rotate,select,view:setView,region:setRegion,configure:(o)=>{options=o;if(!o.motion)awakening.advance(0,false);notifyAwakening();if(flight&&!o.motion)flight.duration=0;lastInteraction=performance.now();stillFrames=0;},storyInset:setStoryInset,dispose:destroy};
}
