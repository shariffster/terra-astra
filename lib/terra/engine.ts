...
export type Engine = { audioState:()=>AudioWorldState; replayGenesis:()=>void; skipGenesis:()=>void; command:(command:WorldCommand)=>Promise<WorldCommandResult>; worldState:()=>WorldState; transform:(open:boolean)=>void; personal:(places:PersonalPlaces|null)=>void; descend:()=>Promise<void>; orbit:()=>void; zoom:(factor:number)=>void; rotate:(dx:number,dy:number)=>void; select:(id:string|null)=>void; configure:(o:ViewOptions)=>void; storyInset:(pixels:number)=>void; compositionPanel:(open:boolean)=>void; view:(view:EarthView)=>void; region:(region:StudyRegion)=>void; dispose:()=>void };
...
export async function createEarth(host:HTMLDivElement,markers:HTMLDivElement,callbacks:Callbacks,signal:AbortSignal):Promise<Engine>{
 ...
 const smoothCameraOffset=new THREE.Vector2();let compositionOpen=false;
 ...
 function updateSea(seconds:number,visibility:number){
  ...
  seaHubs.material.uniforms.opacity.value=alpha*.40*displayedLight.pathLight*displayedLight.cableLight*cableReveal(awakening.elapsed-AWAKENING.cables.start-2,1)*cablePathGates.average;
 ...
 return {audioState,replayGenesis,skipGenesis,command,worldState,transform:setTransformation,personal:setPersonal,descend:async()=>{if(flight||morph||genesisProgress<1||openProgress>0)return;try{personalSettledAt=null;personalArrivalSent=false;clearStory();remembered=null;remembering=false;settledAt=null;arrivalSent=false;activeOpenTarget=undefined;void openViews.setTarget(null);targetId='singapore';scaleTier='city';if(presentation.focus!=='living'){presentation={...presentation,focus:'living'};syncPresentation();}loadingDetail=true;fly(1.2965,103.851,.0018,'city');notifyWorld();try{await ensureCity();}finally{loadingDetail=false;stillFrames=0;notifyWorld();}if(disposed)return;}catch(e){if(!signal.aborted)callbacks.error('Singapore could not load. Your Earth is still here; try entering again.');throw e;}},orbit:()=>{if(flight||morph||stage!=='city')return;clearStory();remembering=!!remembered;settledAt=null;arrivalSent=false;activeOpenTarget=undefined;void openViews.setTarget(null);targetId=null;scaleTier='planet';fly(19,95,homeAltitude(),'orbit');notifyWorld();},zoom,rotate,select,view:setView,region:setRegion,configure:(o)=>{options={...DEFAULT_LIGHT,...o};reducedMotion=matchMedia('(prefers-reduced-motion: reduce)').matches;for(const transition of activityTransitions)transition.update(layerClock,reducedMotion);if(!o.motion)awakening.advance(0,false);notifyAwakening();if(flight&&!o.motion)flight.duration=0;lastInteraction=performance.now();stillFrames=0;},storyInset:setStoryInset,compositionPanel:(open)=>{compositionOpen=open;stillFrames=0;},dispose:destroy};
}
