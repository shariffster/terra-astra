import { finishPreparation, runPreparation, yieldPreparation } from './preparation';
import { advanceTransportClock, travellerPulse, travellerPool, travellerProgress, sampleTravellerPath, localPathOffset, localPathLight, MAX_TRAVELLERS, TRANSPORT_FAMILIES, cloneTransport, type TransportFamily, type TravellerPath, type Traveller } from './transport';
import { gatherAirCorridors, gatherAirCorridorsSteps, networkImportance, networkImportanceSteps } from './network-composition';
import { structureLandLightSteps } from './land-light';
import * as THREE from 'three';
import { loadConnectionAtlas, atlasFlights, atlasMarine } from '../world/connection-atlas';
import { compositionFraming, planetFraming, type PanelBounds } from './composition-framing';
import { createCelestialSetting, rotationRate } from './celestial-setting';
...
import { cablePaths, cableColor, CABLE_SEGMENTS_PER_PATH, cableHubs } from '../world/cables';
import { prepareSmoothCables, prepareSmoothCablesSteps, sampleSmoothCable, marineStrands, marineStrandsSteps } from '../world/smooth-cables';
import { cableFilamentGeometry, cableFilamentGeometrySteps, cableFilamentVertex, cableFilamentFragment, movingPathGeometry, writeMovingPath } from './cable-filaments';
import { seaLanePaths, seaLaneColor } from '../world/sea-lanes';
import { createOceanVolume, prepareCurrentPaths, sampleSuspended, currentPaths, CURRENT_PARTICLES, sampleCurrent, oceanExposure } from '../world/ocean-volume';
...
import { AwakeningTimeline, AWAKENING, awakeningOrder, signalOnset, signalReveal, cableOnsets, cableReveal, travellerArrivals } from './awakening';
...
export type Engine = { audioState:()=>AudioWorldState; beginGenesis:()=>void; replayGenesis:()=>void; skipGenesis:()=>void; command:(command:WorldCommand)=>Promise<WorldCommandResult>; worldState:()=>WorldState; transform:(open:boolean)=>void; personal:(places:PersonalPlaces|null)=>void; descend:()=>Promise<void>; orbit:()=>void; zoom:(factor:number)=>void; rotate:(dx:number,dy:number)=>void; select:(id:string|null)=>void; configure:(o:ViewOptions)=>void; storyInset:(pixels:number)=>void; compositionPanel:(open:boolean,bounds?:PanelBounds)=>void; narrativePanel:(visible:boolean,bounds?:PanelBounds)=>void; compareLight:(light:ViewOptions|null)=>void; view:(view:EarthView)=>void; region:(region:StudyRegion)=>void; dispose:()=>void };
type Callbacks={deferGenesis?:boolean;discovery?:(ready:boolean)=>void;genesis?:(state:GenesisState)=>void;worldState?:(state:WorldState)=>void;transformation?:(state:TransformationState)=>void;personalSettled?:()=>void;stage:(s:Stage)=>void;ready:()=>void;error:(s:string,fatal?:boolean)=>void;coordinates:(lat:number,lon:number)=>void;interact:()=>void;arrival:(storyId:string|null)=>void;view?:(view:EarthView)=>void};
...
uniform float shoreBreath;uniform float regional;uniform float regionMix;uniform vec3 regionFocus;uniform float regionOuter;uniform float regionInner;
...
export async function createEarth(host:HTMLDivElement,markers:HTMLDivElement,callbacks:Callbacks,signal:AbortSignal,initialLight?:ViewOptions):Promise<Engine>{
 ...
 const gateTextures:{texture:THREE.DataTexture;gates:ActivityTransitions;bytes:Uint8Array}[]=[];
 const geometries:THREE.BufferGeometry[]=[];const materials:THREE.Material[]=[];const clouds:Cloud[]=[];
 const homeAltitude=()=>{const w=host.clientWidth,h=host.clientHeight,r=Math.min(w*(w<700?.37:w<1100?.38:.30),h*.38);return Math.max(2.15,Math.sqrt(1+Math.pow(h/(2*Math.tan(21*R)*r),2))-1);};
 const heldKeys=new Set<string>();
 let shapeTimer:ReturnType<typeof setTimeout>|null=null;let shapeRequest:AbortController|null=null;
 ...
 // Track the auto-spin contribution with the same easing as Earth, so manual
 // inspection can move the observing frame without dragging it during idle spin.
 let skySpin=0,skySpinTarget=0;
 ...
 let genesisHeld=!!callbacks.deferGenesis,firstFrameReady=false;
 const awakening=new AwakeningTimeline();let discoveryReady=false;
 function notifyAwakening(){if(discoveryReady!==awakening.discovery){discoveryReady=awakening.discovery;callbacks.discovery?.(discoveryReady);}host.dataset.awakening=awakening.state;host.dataset.awakeningSeconds=awakening.elapsed.toFixed(3);host.dataset.idleDegreesPerSecond=rotationRate(options,(performance.now()-lastInteraction)/1000,awakening.idleRate/.84).toFixed(3);}
 ...
 let options:ViewOptions={...DEFAULT_LIGHT,...initialLight,transport:initialLight?.transport??cloneTransport(),motion:reducedMotion?false:initialLight?.motion??true};
 ...
 const celestial=createCelestialSetting(host,()=>{stillFrames=0;},fallback?undefined:renderer as THREE.WebGLRenderer);
 const celestialCentre=new THREE.Vector3();
 ...
 function cloud(data:Float32Array,tint:string,feather:boolean|((lon:number,lat:number)=>number)=false){return finishPreparation(cloudSteps(data,tint,feather));}
 function* cloudSteps(data:Float32Array,tint:string,feather:boolean|((lon:number,lat:number)=>number)=false):Generator<void,Cloud & {points:THREE.Points},void>{const count=data.length/6;const p=new Float32Array(count*3),b=new Float32Array(count),s=new Float32Array(count),ph=new Float32Array(count);for(let i=0;i<count;i++){p.set(data.subarray(i*6,i*6+3),i*3);b[i]=data[i*6+3]*(feather?(typeof feather==='function'?feather:cityFeather)(Math.atan2(p[i*3],p[i*3+2])/R,Math.atan2(p[i*3+1],Math.hypot(p[i*3],p[i*3+2]))/R):1);s[i]=data[i*6+4];ph[i]=data[i*6+5];if((i+1)%2048===0)yield;}
 const g=geometry(p);const ids=new Float32Array(count),seeds=new Float32Array(count*3);for(let i=0;i<count;i++){ids[i]=particleOffset+i+1;seeds.set([genesisSeed(ids[i],1),genesisSeed(ids[i],2),genesisSeed(ids[i],3)],i*3);if((i+1)%2048===0)yield;}particleOffset+=count;g.setAttribute('particleId',new THREE.BufferAttribute(ids,1));g.setAttribute('genesisSeed',new THREE.BufferAttribute(seeds,3));g.setAttribute('brightness',new THREE.BufferAttribute(b,1));g.setAttribute('starSize',new THREE.BufferAttribute(s,1));g.setAttribute('phase',new THREE.BufferAttribute(ph,1));g.setAttribute('revealAt',new THREE.BufferAttribute(new Float32Array(count),1));
 const m=new THREE.ShaderMaterial({vertexShader:vertex,fragmentShader:fragment,uniforms:{shoreBreath:{value:0},genesis:{value:1},genesisEnabled:{value:0},genesisExposure:{value:1},genesisSize:{value:1},tint:{value:new THREE.Color(tint)},opacity:{value:1},glow:{value:1},time:{value:0},motion:{value:1},pixelRatio:{value:renderer.getPixelRatio()},zoomFactor:{value:1},sizeScale:{value:1},reveal:{value:1},sparkle:{value:1},signature:{value:-1},shell:{value:0},opening:{value:0},openingFrame:{value:openFrame},personalStar:{value:0},terrainKind:{value:0},terrainLand:{value:1},terrainFloor:{value:1},terrainStudy:{value:0},terrainReady:{value:0},seaMotion:{value:0},spatial:{value:0},depthMix:{value:0},cutaway:{value:0},cutNormal:{value:cutNormal},cutFacing:{value:cutFacing},cameraDistance:{value:3},soft:{value:0},regional:{value:0},regionMix:{value:0},regionFocus:{value:new THREE.Vector3(0,0,1)},regionOuter:{value:.78},regionInner:{value:.975}},transparent:true,depthWrite:false,depthTest:true,blending:THREE.AdditiveBlending});Object.assign(m.defaultAttributeValues,{terrainData:[0,0,0,0]});materials.push(m);const points=new THREE.Points(g,m);points.frustumCulled=false;earth.add(points);const c={points,material:m,count};clouds.push(c);return c;}
 ...
 function terrainCloud(c:Cloud,kind:number,gradients=false){finishPreparation(terrainCloudSteps(c,kind,gradients));}
 function* terrainCloudSteps(c:Cloud,kind:number,gradients=false):Generator<void,void,void>{
  ...
  for(let i=0;i<p.count;i++){
   ...
   field.set([ax/len,ay/len,az/len,metres/11000],i*4);if((i+1)%512===0)yield;
  }
  c.points.geometry.setAttribute('terrainData',new THREE.BufferAttribute(field,4));
 }
 function* liftSteps(data:Float32Array):Generator<void,Float32Array,void>{const out=data.slice();for(let i=0;i<out.length;i+=6){const v=new THREE.Vector3(out[i],out[i+1],out[i+2]);const h=sampleElevation(elevation,1440,720,Math.atan2(v.x,v.z)/R,Math.atan2(v.y,Math.hypot(v.x,v.z))/R);v.normalize().multiplyScalar(reliefRadius(Math.max(0,h))+.00004);out.set(v.toArray(),i);if((i/6+1)%1024===0)yield;}return out;}
 ...
 let atlas:Awaited<ReturnType<typeof loadConnectionAtlas>>=null;
 ...
 const smoothCameraOffset=new THREE.Vector2();let compositionOpen=false,narrativeVisible=true;let compositionBounds:PanelBounds|null=null,openingBounds:PanelBounds|null=null;
 ...
 const destroy=()=>{if(disposed)return;disposed=true;shapeRequest?.abort();if(shapeTimer!==null)clearTimeout(shapeTimer);resolverRequest?.abort();openViews?.dispose();awakening.dispose();if(warmupTimer!==null)clearTimeout(warmupTimer);for(const resolve of idleWaiters.splice(0))resolve(false);cancelAnimationFrame(raf);celestial.dispose();observer.disconnect();host.removeEventListener('pointerdown',down);host.removeEventListener('pointermove',move);host.removeEventListener('pointerup',up);host.removeEventListener('pointercancel',cancel);host.removeEventListener('wheel',wheel);renderer.domElement.removeEventListener('keydown',key);window.removeEventListener('keyup',keyUp);window.removeEventListener('blur',clearHeld);renderer.domElement.removeEventListener('blur',clearHeld);heldKeys.clear();renderer.domElement.removeEventListener('webglcontextlost',contextLost);document.removeEventListener('visibilitychange',visibility);for(const g of geometries)g.dispose();for(const m of materials)m.dispose();for(const {texture} of gateTextures)texture.dispose();renderer.dispose();renderer.domElement.remove();};
 ...
 async function prepare<T>(phase:string,steps:Generator<void,T,void>):Promise<T>{
  host.dataset.startupPhase=phase;
  try{await yieldPreparation(signal);return await runPreparation(steps,signal);}catch(error){destroy();throw error;}
 }
 async function breathe(){try{await yieldPreparation(signal);}catch(error){destroy();throw error;}}
 async function prepareCloud(data:Float32Array,tint:string){const c=await prepare('earth-particles',cloudSteps(data,tint));c.material.uniforms.spatial.value=1;c.points.userData.spatial=true;c.points.userData.sampleBudget=22000;c.points.userData.spatialExposure=1.8;return c;}
 try{
 host.dataset.startupPhase='assets';
 ...
 const atlasRequest=loadConnectionAtlas(signal);
 ...
 host.dataset.startupPhase='earth';await breathe();
 land=await prepareCloud(await prepare('land-relief',liftSteps(data[0])),'#b9b0a0');
 coast=await prepareCloud(await prepare('coast-relief',liftSteps(data[1])),'#e6dabe');
 lights=await prepareCloud(await prepare('city-relief',liftSteps(data[2])),'#eed7a8');
 coastLines=lines(data[3],'#789dab');borders=lines(data[4],'#89a8b5');
 for(const c of [land,coast,lights]){c.points.userData.sampleBudget=5000;c.points.userData.spatialExposure=1;}
 reliefLand=await prepareCloud(data[5],'#8f9598');reliefOcean=await prepareCloud(data[6],'#718084');body=await prepareCloud(data[7],'#a79a83');
 interior=await prepareCloud(data[8],'#b4a17f');haze=await prepareCloud(data[9],'#887f75');halo=await prepareCloud(data[10],'#c7d0d6');
 haze.material.uniforms.soft.value=1;reliefOcean.material.uniforms.seaMotion.value=1;
 terrainCloud(land,1);terrainCloud(coast,3);terrainCloud(lights,4);
 await prepare('land-normals',terrainCloudSteps(reliefLand,1,true));await prepare('ocean-normals',terrainCloudSteps(reliefOcean,2,true));
 const humanData=await humanRequest;atlas=await atlasRequest;
 host.dataset.networkAtlas=atlas?'ready':'unavailable';
 if(signal.aborted){destroy();throw new DOMException('Aborted','AbortError');}
 if(humanData){for(const data of humanData)await prepare('land-light',structureLandLightSteps(data));
 populationField=await prepareCloud(humanData[0],'#8eabc6');footprintField=await prepareCloud(humanData[1],'#9786ae');for(const c of [populationField,footprintField]){terrainCloud(c,1);await breathe();c.points.userData.humanField=true;c.points.userData.shimmer=.06;c.material.uniforms.opacity.value=0;}}
 ...
 const historicalSource=atlasFlights(atlas?.air.filter((_,i)=>!fallback||i%6===0)??[]);
 let historicalFlights=await prepare('air-corridors',gatherAirCorridorsSteps(historicalSource,(lon,lat)=>sampleElevation(elevation,1440,720,lon,lat),options.transport.aircraft.bundle,options.transport.aircraft.roundness));
 let flightPaths=[...prepareFlightPaths(aircraftCorridors,(lon,lat)=>sampleElevation(elevation,1440,720,lon,lat)),...historicalFlights];
 const allSeaPaths=[...seaLanePaths,...atlasMarine(atlas?.sea??[],true)];
 const allCablePaths=[...cablePaths,...atlasMarine(atlas?.cables??[],false)];
 host.dataset.networkCounts=JSON.stringify({air:flightPaths.length,sea:allSeaPaths.length,cables:allCablePaths.length,aircraft:aircraftSignals.length,ships:shipSignals.length});
 let smoothShipping=await prepare('shipping-corridors',prepareSmoothCablesSteps(allSeaPaths,(lon,lat)=>sampleElevation(elevation,1440,720,lon,lat),1.002,options.transport.ships));
 let smoothCables=await prepare('cable-corridors',prepareSmoothCablesSteps(allCablePaths,(lon,lat)=>sampleElevation(elevation,1440,720,lon,lat),undefined,options.transport.cables));
 const orbitPaths=satelliteSignals.map(record=>{const positions=new Float32Array(513*3),progress=new Float32Array(513);for(let k=0;k<=512;k++){const angle=k/512*Math.PI*2;progress[k]=k/512;for(let j=0;j<3;j++)positions[k*3+j]=record.radius*(record.basisA[j]*Math.cos(angle)+record.basisB[j]*Math.sin(angle));}return {id:record.id,positions,progress,radius:record.radius};});
 const familyPaths=(layer:TransportFamily):readonly TravellerPath[]=>layer==='aircraft'?flightPaths:layer==='ships'?smoothShipping:layer==='cables'?smoothCables:orbitPaths;
 ...
 function movingWindow(layer:TransportFamily,gates:ActivityTransitions){
  ...
  if(fallback)for(let i=0;i<data.length;i+=6)data.set([0,0,1,0,.55,0],i);
  const c:Cloud=cloud(data,familyTint(layer));c.points.userData.shell=layer+'-window';c.points.userData.shimmer=0;c.material.uniforms.shell.value=layer==='cables'?2:1;c.points.userData.fallbackExposure=.8;
  if(!fallback){const original=c.points;earth.remove(original);original.geometry.dispose();geometries.splice(geometries.indexOf(original.geometry),1);
   const g=movingPathGeometry(MAX_TRAVELLERS,WINDOW_SAMPLES);geometries.push(g);const bytes=new Uint8Array(MAX_TRAVELLERS*4).fill(255),texture=new THREE.DataTexture(bytes,MAX_TRAVELLERS,1,THREE.RGBAFormat);texture.needsUpdate=true;gateTextures.push({texture,bytes,gates});
   ...
   Object.assign(c.material.uniforms,{localPath:{value:1},lineSoftness:{value:.55},pathKind:{value:layer==='cables'?0:layer==='ships'?1:2},resolution:{value:new THREE.Vector2(host.clientWidth,host.clientHeight)},awakeningTime:{value:100},routeGate:{value:texture},routeSize:{value:new THREE.Vector2(MAX_TRAVELLERS,1)},secondaryLight:{value:1},richness:{value:1},drawDuration:{value:0},fadeDuration:{value:1}});
   const mesh=new THREE.Mesh(g,c.material);mesh.userData={...original.userData};mesh.frustumCulled=false;earth.add(mesh);c.points=mesh;
  }
  if(layer==='cables'){c.points.userData.network=true;c.points.userData.spatial=true;c.material.uniforms.spatial.value=1;terrainCloud(c,4);}
  return c;
 }
 function signalView(layer:TransportFamily){
  const records=travellerPool(layer,familyPaths(layer),familySources(layer),options.transport[layer].distribution,satelliteSignals),data=new Float32Array(MAX_TRAVELLERS*TRAIL_SAMPLES*6),arrival=travellerArrivals(layer,options.transport[layer].count*options.travellerVolume,MAX_TRAVELLERS);
  const material=materialFor(layer);
  for(let i=0;i<MAX_TRAVELLERS;i++){for(let k=0;k<TRAIL_SAMPLES;k++)data.set([0,0,1,0,k===0?material.size:(layer==='ships'?1.25:.85)*(1-k/TRAIL_SAMPLES),i*.73%6.28],(i*TRAIL_SAMPLES+k)*6);}
  const c=cloud(data,familyTint(layer));
  // Soften tails independently: the bright moving head stays recognisable.
  const trailPart=Float32Array.from({length:MAX_TRAVELLERS*TRAIL_SAMPLES},(_,i)=>i%TRAIL_SAMPLES===0?0:1);
  ...
  return {clock:{travel:0,pulse:0},lastTime:0,pulseMix:options.transport[layer].pulse?1:0,records,cloud:c,window:movingWindow(layer,localGates),positions:c.points.geometry.getAttribute('position').array as Float32Array,brightness:c.points.geometry.getAttribute('brightness') as THREE.BufferAttribute,arrival,trailCount:TRAIL_SAMPLES,layer,gates,localGates,tailGates,population:new Float32Array(MAX_TRAVELLERS),distribution:options.transport[layer].distribution,activity:0,drawCount:0};
 }
 const signalViews:ReturnType<typeof signalView>[]=[];
 for(const family of ['satellites','aircraft','ships','cables'] as const){await breathe();signalViews.push(signalView(family));}
 const signalOutput=new Float64Array(3),windowXYZ=new Float32Array(WINDOW_SAMPLES*3),windowLight=new Float32Array(WINDOW_SAMPLES);
 const cableView=signalViews[3],seaPulses=cableView.cloud;
 function sampleTraveller(view:typeof signalViews[number],record:Traveller,seconds:number,out:Float64Array){
  const path=familyPaths(view.layer)[record.route];sampleTravellerPath(path,travellerProgress(record,seconds),out);
  if(view.layer==='aircraft')liftFlight(path as typeof flightPaths[number],depth,terrain.land*(1+TERRAIN.grazingLand*terrain.study),out);
 }
 function updateSignals(nowSeconds:number,visibility:number,dt:number){const exposure=livingExposure(alt);for(const v of signalViews){
  const cfg=options.transport[v.layer];
  advanceTransportClock(v.clock,Math.max(0,nowSeconds-v.lastTime),cfg);v.lastTime=nowSeconds;
  v.pulseMix+=(Number(cfg.pulse)-v.pulseMix)*(reducedMotion||!options.motion?1:1-Math.exp(-dt*4));
  const travelSeconds=v.clock.travel,m=materialFor(v.layer),scale=displayedLight[familyKey(v.layer)],level=v.layer==='satellites'?exposure.orbit:v.layer==='aircraft'?exposure.air:v.layer==='cables'?exposure.cables:exposure.ships;
  v.cloud.material.uniforms.opacity.value=visibility*displayedLight.travellerLight*scale*m.opacity*level*cfg.travellerLight*Math.min(1,v.gates.average*4);
  v.window.material.uniforms.opacity.value=visibility*displayedLight.pathLight*scale*level*cfg.pathLight*.9;
  v.window.material.uniforms.soft.value=cfg.softness*.5;
  if(!fallback){v.window.material.uniforms.resolution.value.set(host.clientWidth,host.clientHeight);v.window.material.uniforms.lineSoftness.value=cfg.softness;}
  v.cloud.material.uniforms.soft.value=cfg.softness;
  const windowActive=v.localGates.average>.0001;v.window.material.uniforms.opacity.value*=windowActive?1:0;
  const count=cfg.count*displayedLight.travellerVolume;let active=0,full=0,last=0;v.activity=0;
  for(let i=0;i<v.records.length;i++){
   const target=smoothUnit(count-i),population=v.population[i]+(target-v.population[i])*(reducedMotion||!options.motion?1:1-Math.exp(-dt*(1.4+activitySeed(v.records[i].id)*2)));v.population[i]=population;
   const age=awakening.elapsed-v.arrival.onsets[i],reveal=smoothUnit(age/v.arrival.fade),weight=population*reveal;
   if(weight<.0001){for(let k=0;k<TRAIL_SAMPLES;k++)v.brightness.setX(i*TRAIL_SAMPLES+k,0);continue;}
   active++;if(reveal>=1)full++;last=i+1;const record=v.records[i],pulse=travellerPulse(v.clock.pulse,record.phase,cfg.pulseDepth*v.pulseMix),gate=v.gates.values[i]*weight;
   v.activity+=gate/Math.max(1,cfg.count);
   for(let k=0;k<TRAIL_SAMPLES;k++){
    const fraction=k/(TRAIL_SAMPLES-1),tail=Math.pow(1-fraction,1.7),tailGate=v.tailGates.values[i]*weight;
    v.brightness.setX(i*TRAIL_SAMPLES+k,pulse*(k===0?m.head*gate:cfg.tail>0?m.tail*tail*tailGate*cfg.trailLight:0));
    sampleTraveller(v,record,travelSeconds-record.trailSeconds*fraction*cfg.tail,signalOutput);v.positions.set(signalOutput,(i*TRAIL_SAMPLES+k)*3);
   }
   const windowGate=v.localGates.values[i]*weight;
   if(windowActive){for(let k=0;k<WINDOW_SAMPLES;k++){const fraction=k/(WINDOW_SAMPLES-1);sampleTraveller(v,record,travelSeconds+localPathOffset(fraction,cfg.ahead,cfg.behind,record.period),signalOutput);windowXYZ.set(signalOutput,k*3);windowLight[k]=windowGate*localPathLight(fraction);}
    if(fallback){const p=v.window.points.geometry.getAttribute('position'),b=v.window.points.geometry.getAttribute('brightness');for(let k=0;k<WINDOW_SAMPLES;k++){p.setXYZ(i*WINDOW_SAMPLES+k,windowXYZ[k*3],windowXYZ[k*3+1],windowXYZ[k*3+2]);b.setX(i*WINDOW_SAMPLES+k,windowLight[k]);}}
    else writeMovingPath(v.window.points.geometry,i,windowXYZ,windowLight);
   }
  }
  v.activity=v.activity>.999999?1:v.activity;v.drawCount=last*TRAIL_SAMPLES;v.cloud.points.geometry.setDrawRange(0,v.drawCount);v.brightness.needsUpdate=true;v.cloud.points.geometry.getAttribute('position').needsUpdate=true;
  v.window.points.geometry.setDrawRange(0,windowActive?last*(fallback?WINDOW_SAMPLES:(WINDOW_SAMPLES-1)*6):0);
  for(const name of windowActive?(fallback?['position','brightness']:['position','previous','next','localLight']):[])v.window.points.geometry.getAttribute(name).needsUpdate=true;
  host.dataset[v.layer+'Awake']=String(active);host.dataset[v.layer+'Full']=String(full);host.dataset[v.layer+'TravellersFull']=String(full);host.dataset[v.layer+'TravellerCount']=String(active);
  if(v.layer==='cables')host.dataset.cablePulseJourneys=String(Math.round(active*v.gates.average));
 }}
 ...
 let lastWaterTime=-Infinity,lastCurrentTime=0,currentSeconds=0;
 function updateWater(seconds:number,visibility:number){
  currentSeconds+=Math.max(0,seconds-lastCurrentTime)*options.currentSpeed;lastCurrentTime=seconds;
  const exposure=oceanExposure(alt,tilt,cut)*visibility*depth*awakening.ocean;
  suspended.material.uniforms.opacity.value=exposure*2.3*displayedLight.oceanLight;currents.material.uniforms.opacity.value=(exposure+visibility*awakening.ocean*.12)*1.4*displayedLight.oceanLight*displayedLight.currentLight;
  suspended.points.geometry.setDrawRange(0,Math.floor(suspended.count*(host.clientWidth<700?.58:1)));
  if((seconds===0||Math.abs(seconds-lastWaterTime)>=1/30)&&(exposure>.001||displayedLight.currentLight>.001)){
   ...
   for(let i=0;i<currentPaths.length;i++)for(let k=0;k<CURRENT_PARTICLES;k++){const light=sampleCurrent(i,k,currentSeconds,elevation,waterSample,smoothCurrents),n=i*CURRENT_PARTICLES+k;currentPosition.setXYZ(n,waterSample[0],waterSample[1],waterSample[2]);currentBrightness.setX(n,light);}
 ...
 const cablePathGates=new ActivityTransitions(allCablePaths.map(p=>p.id),'pathways');
 const cableTravellerGates=cableView.gates;
 const orbitPathGates=new ActivityTransitions(orbitPaths.map(p=>p.id),'pathways',false);
 const activityTransitions=[...signalViews.flatMap(v=>[v.gates,v.localGates,v.tailGates]),airPathGates,shipPathGates,cablePathGates,orbitPathGates,focusTransition,populationFocus,footprintFocus,connectionsFocus];
 ...
 async function ribbon(c:Cloud,paths:readonly PathGeometry[],sources:readonly {intensity:number}[],onsets:Float32Array,kind:number,gates:ActivityTransitions,preparedImportance?:number[]){
  ...
  const bases=paths.filter(p=>!('strand' in p)||!p.strand) as (PathGeometry&{id:string})[];const importance=preparedImportance??await prepare('connection-hierarchy',networkImportanceSteps(bases,sources,kind<1.5));const g=await prepare('connection-ribbons',cableFilamentGeometrySteps(paths,sources.map((s,i)=>({...s,importance:importance[i]})),onsets));geometries.push(g);
  ...
  Object.assign(c.material.uniforms,{localPath:{value:0},lineSoftness:{value:.55},resolution:{value:new THREE.Vector2(host.clientWidth,host.clientHeight)},awakeningTime:{value:0},pathKind:{value:kind},routeGate:{value:texture},routeSize:{value:new THREE.Vector2(columns,rows)},secondaryLight:{value:options.strandLight},richness:{value:DEFAULT_LIGHT.pathVolume},drawDuration:{value:kind===0?AWAKENING.cables.draw:2.2},fadeDuration:{value:kind===0?AWAKENING.cables.fade:1.1}});
 ...
 async function pathway(paths:readonly PathGeometry[],sources:readonly {intensity:number}[],onsets:Float32Array,tint:string,kind:number,gates:ActivityTransitions){
  const data:number[]=[],routeIndices:number[]=[],progress:number[]=[];const importance=await prepare('connection-hierarchy',networkImportanceSteps(paths as (PathGeometry&{id:string})[],sources,kind<1.5));
  ...
  if(!fallback){const strands=kind===1?await prepare('shipping-strands',marineStrandsSteps(paths as typeof smoothShipping,(lon,lat)=>sampleElevation(elevation,1440,720,lon,lat),true,options.strandSpread)):paths;await ribbon(c,strands,sources,onsets,kind,gates,importance);}
  // The material does not move; the travelling light supplies local highlights.
  return {cloud:c,gates,onsets,kind,sources,base,routeIndices,progress,importance};
 }
 const airLanes=await pathway(flightPaths,[...aircraftCorridors.map((_,i)=>({intensity:i%13===12?0:.72})),...historicalFlights.map(p=>({intensity:p.intensity,tier:p.tier,threshold:p.threshold}))],airOnsets,signalColors.aircraft,2,airPathGates);
 const seaLaneView=await pathway(smoothShipping,allSeaPaths.map((p,i)=>({intensity:i<seaLanePaths.length&&i%19===18?0:p.intensity,tier:p.tier,threshold:i<seaLanePaths.length?0:.10+.66*((i-seaLanePaths.length)/Math.max(1,allSeaPaths.length-seaLanePaths.length))})),shipOnsets,seaLaneColor,1,shipPathGates),seaLanes=seaLaneView.cloud;
 seaLanes.points.userData.seaLanes=true;
 const orbitLanes=await pathway(orbitPaths,orbitPaths.map(()=>({intensity:.65})),Float32Array.from(orbitPaths,()=>2.5),signalColors.satellites,3,orbitPathGates);
 orbitLanes.cloud.points.userData.shell='orbit-lanes';
 function updatePathway(view:Awaited<ReturnType<typeof pathway>>,visibility:number){
  const c=view.cloud;c.material.uniforms.opacity.value=visibility*Math.min(1,view.gates.average*4);
  if(!fallback){c.material.uniforms.secondaryLight.value=displayedLight.strandLight;c.material.uniforms.richness.value=displayedLight.pathVolume;c.material.uniforms.awakeningTime.value=awakening.elapsed;c.material.uniforms.resolution.value.set(host.clientWidth,host.clientHeight);return;}
  ...
  for(let k=0;k<view.routeIndices.length;k++){
   ...
   b.setX(k,displayedLight.pathVolume*view.sources[i].intensity*view.importance[i]*smoothUnit((age-progress*2.2)/1.1)*smoothUnit((view.gates.values[i]-progress*.3)/.7));
 ...
 function syncPresentation(){
  ...
  for(const v of signalViews){const cfg=options.transport[v.layer],enabled=activity&&layerFlags[v.layer];
   v.gates.set(enabled&&presentation.travellers&&cfg.travellers,layerClock,reducedMotion);
   v.localGates.set(enabled&&presentation.pathways&&cfg.pathways&&cfg.mode==='local',layerClock,reducedMotion);
   v.tailGates.set(enabled&&presentation.pathways&&presentation.travellers&&cfg.pathways&&cfg.travellers,layerClock,reducedMotion);
  }
  for(const [family,gates] of [['aircraft',airPathGates],['ships',shipPathGates],['cables',cablePathGates],['satellites',orbitPathGates]] as const){const cfg=options.transport[family];gates.set(activity&&presentation.pathways&&layerFlags[family]&&cfg.pathways&&cfg.mode==='full',layerClock,reducedMotion);}
 ...
 function syncTransport(){
  for(const v of signalViews){const cfg=options.transport[v.layer];if(v.distribution!==cfg.distribution){v.records=travellerPool(v.layer,familyPaths(v.layer),familySources(v.layer),cfg.distribution,satelliteSignals);v.population.fill(0);v.distribution=cfg.distribution;}}
  scheduleShape();
 }
 // Cable-only rounded geography; the seated stroke and signal lights share it.
 const cableSample=new Float64Array(3),cableStars:number[]=[];
 ...
 const seaOnsets=cableOnsets(allCablePaths);
 for(let i=0;fallback&&i<allCablePaths.length;i++){
 ...
 const seaFilaments:Cloud=cloud(new Float32Array(cableStars),cableColor);
 if(!fallback)await ribbon(seaFilaments,await prepare('cable-strands',marineStrandsSteps(smoothCables,(lon,lat)=>sampleElevation(elevation,1440,720,lon,lat),false,options.strandSpread)),allCablePaths.map((p,i)=>({...p,threshold:i<cablePaths.length?0:.12+.64*((i-cablePaths.length)/Math.max(1,allCablePaths.length-cablePaths.length))})),seaOnsets,0,cablePathGates);
 ...
 for(const c of [seaFilaments,seaHubs]){
 ...
 function updateSea(seconds:number,visibility:number){
  for(const {texture,gates,bytes} of gateTextures){let changed=false;for(let i=0;i<gates.values.length;i++){const v=Math.round(gates.values[i]*255);if(bytes[i*4]!==v){bytes[i*4]=v;changed=true;}}if(changed)texture.needsUpdate=true;}
  const exposure=livingExposure(alt);
  updatePathway(airLanes,visibility*exposure.air*.57*displayedLight.pathLight*displayedLight.airLight*options.transport.aircraft.pathLight);
  updatePathway(seaLaneView,visibility*exposure.lanes*.66*displayedLight.pathLight*displayedLight.seaLight*options.transport.ships.pathLight);
  updatePathway(orbitLanes,visibility*exposure.orbit*.38*displayedLight.pathLight*displayedLight.orbitLight*options.transport.satellites.pathLight);
  for(const [c,f] of [[airLanes.cloud,'aircraft'],[seaLaneView.cloud,'ships'],[orbitLanes.cloud,'satellites'],[seaFilaments,'cables']] as const)if(!fallback)c.material.uniforms.lineSoftness.value=options.transport[f].softness;
  ...
  seaFilaments.material.uniforms.opacity.value=(fallback?displayedLight.pathVolume:1)*alpha*.88*displayedLight.pathLight*displayedLight.cableLight*options.transport.cables.pathLight*Math.min(1,cablePathGates.average*4);
  ...
  if(!fallback){seaFilaments.material.uniforms.secondaryLight.value=displayedLight.strandLight;seaFilaments.material.uniforms.richness.value=displayedLight.pathVolume;seaFilaments.material.uniforms.awakeningTime.value=awakening.elapsed;seaFilaments.material.uniforms.resolution.value.set(host.clientWidth,host.clientHeight);}
  let active=0,full=0;
  for(let i=0;i<allCablePaths.length;i++){
   ...
   if(fallback)for(let k=0;k<CABLE_SEGMENTS_PER_PATH;k++){
    ...
    cableBrightness.setX(i*CABLE_SEGMENTS_PER_PATH+k,(endpoint?.65:.54)*allCablePaths[i].intensity*cableReveal(age,k/(CABLE_SEGMENTS_PER_PATH-1))*smoothUnit((cablePathGates.values[i]-k/(CABLE_SEGMENTS_PER_PATH-1)*.3)/.7));
   }
  }
  host.dataset.cableHubs=String(cableHubs.length);
 ...
 let shapeSignature=JSON.stringify((['aircraft','ships','cables'] as const).map(f=>[options.transport[f].bundle,options.transport[f].roundness,f==='aircraft'?0:options.strandSpread]));
 let pendingShape=shapeSignature;
 ...
 function rebuildPath(view:Awaited<ReturnType<typeof pathway>>,paths:readonly TravellerPath[],surface=false){
 ...
 function scheduleShape(){
  const settings=cloneTransport(options.transport),spread=options.strandSpread;
  const next=JSON.stringify((['aircraft','ships','cables'] as const).map(f=>[settings[f].bundle,settings[f].roundness,f==='aircraft'?0:spread]));
  if(next===pendingShape)return;
  shapeRequest?.abort();if(shapeTimer!==null){clearTimeout(shapeTimer);shapeTimer=null;}
  pendingShape=next;
  const report=(state:string)=>{host.dataset.routeShape=state;window.dispatchEvent?.(new CustomEvent('terra-route-shape',{detail:state}));};
  if(next===shapeSignature){report('ready');return;}
  report('working');
  shapeTimer=setTimeout(async()=>{shapeTimer=null;if(disposed)return;
   const request=new AbortController();shapeRequest=request;const prepared:THREE.BufferGeometry[]=[],apply:(()=>void)[]=[];
   const step=<T,>(steps:Generator<void,T,void>)=>runPreparation(steps,request.signal);
   try{const old=JSON.parse(shapeSignature) as number[][],height=(lon:number,lat:number)=>sampleElevation(elevation,1440,720,lon,lat);
    for(const [index,f] of (['aircraft','ships','cables'] as const).entries()){
     const cfg=settings[f],shape={bundle:cfg.bundle,roundness:cfg.roundness},pathChanged=old[index][0]!==cfg.bundle||old[index][1]!==cfg.roundness;
     if(!pathChanged&&(f==='aircraft'||old[index][2]===spread))continue;
     if(f==='aircraft'){
      const gathered=await step(gatherAirCorridorsSteps(historicalSource,height,cfg.bundle,cfg.roundness)),paths=[...flightPaths.slice(0,aircraftCorridors.length),...gathered];
      if(fallback)apply.push(()=>{historicalFlights=gathered;flightPaths=paths;rebuildPath(airLanes,paths);});
      else{const importance=await step(networkImportanceSteps(paths,airLanes.sources,false)),g=await step(cableFilamentGeometrySteps(paths,airLanes.sources.map((p,i)=>({...p,importance:importance[i]})),airOnsets));prepared.push(g);apply.push(()=>{historicalFlights=gathered;flightPaths=paths;replaceGeometry(airLanes.cloud,g);});}
     }else{
      const surface=f==='ships',sources=surface?allSeaPaths:allCablePaths;
      const paths=pathChanged?await step(prepareSmoothCablesSteps(sources,height,surface?1.002:undefined,shape)):(surface?smoothShipping:smoothCables);
      if(fallback){apply.push(()=>{if(surface){smoothShipping=paths;rebuildPath(seaLaneView,paths,true);}else{smoothCables=paths;const p=seaFilaments.points.geometry.getAttribute('position');for(let i=0;i<paths.length;i++)for(let k=0;k<CABLE_SEGMENTS_PER_PATH;k++){sampleSmoothCable(paths[i],k/(CABLE_SEGMENTS_PER_PATH-1),cableSample);p.setXYZ(i*CABLE_SEGMENTS_PER_PATH+k,cableSample[0],cableSample[1],cableSample[2]);}p.needsUpdate=true;}});continue;}
      const importance=await step(networkImportanceSteps(paths,surface?seaLaneView.sources:allCablePaths,true));
      const strands=await step(marineStrandsSteps(paths,height,surface,spread));
      const styled=surface?seaLaneView.sources.map((p,i)=>({...p,importance:importance[i]})):allCablePaths.map((p,i)=>({...p,importance:importance[i],threshold:i<cablePaths.length?0:.12+.64*((i-cablePaths.length)/Math.max(1,allCablePaths.length-cablePaths.length))}));
      const g=await step(cableFilamentGeometrySteps(strands,styled,surface?shipOnsets:seaOnsets));prepared.push(g);
      apply.push(()=>{if(surface){smoothShipping=paths;replaceGeometry(seaLanes,g);}else{smoothCables=paths;replaceGeometry(seaFilaments,g);}});
     }
    }
    request.signal.throwIfAborted();if(disposed)throw new DOMException('Disposed','AbortError');
    for(const commit of apply)commit();prepared.length=0;shapeSignature=next;report('ready');stillFrames=0;
   }catch(error){for(const g of prepared)g.dispose();if(!request.signal.aborted&&!disposed){pendingShape=shapeSignature;report('error');console.error('Route shaping could not finish',error);}}
   finally{if(shapeRequest===request)shapeRequest=null;}
  },250);
 }
 function resetAwakening(){awakening.reset();time=0;currentSeconds=0;lastCurrentTime=0;lastWaterTime=-Infinity;for(const v of signalViews){v.clock={travel:0,pulse:0};v.lastTime=0;v.arrival=travellerArrivals(v.layer,options.transport[v.layer].count*options.travellerVolume,MAX_TRAVELLERS);v.population.fill(0);v.cloud.points.geometry.setDrawRange(0,0);v.cloud.material.uniforms.opacity.value=0;v.brightness.array.fill(0);v.brightness.needsUpdate=true;}updateSea(0,0);updateWater(0,0);notifyAwakening();}
 ...
 function worldState():WorldState{return {targetId,tier:scaleTier,busy:isBusy(),genesis:genesisState(genesisProgress),layers:{...layerFlags},presentation:{...presentation},humanFields:populationField&&footprintField?'ready':'unavailable',connectionAtlas:atlas?'ready':'unavailable',...(activeOpenTarget?{resolvedTarget:activeOpenTarget}:{}),...(openContext?{open:openContext}:{}),resolving};}
 ...
 function beginGenesis(){if(disposed||!genesisHeld)return;genesisHeld=false;genesisStarted=performance.now();lastTime=genesisStarted;stillFrames=0;}
 function skipGenesis(){if(disposed||graphicsLost||genesisProgress>=1)return;genesisHeld=false;genesisProgress=1;awakening.settle(options.motion);lastTime=performance.now();stillFrames=0;notifyAwakening();notifyGenesis(true);}
 function replayGenesis(){if(disposed||graphicsLost||loadingDetail)return;genesisHeld=false;journeyEpoch++;lookupSequence++;resolverRequest?.abort();resolving=false;activeOpenTarget=undefined;void openViews.setTarget(null);for(const resolve of idleWaiters.splice(0))resolve(false);resetAwakening();heldKeys.clear();pointer.clear();flight=null;morph=null;clearStory();highlight(null);remembering=false;openProgress=0;openTarget=false;resetView();targetId=null;scaleTier='planet';lat=targetLat=19;lon=targetLon=95;skySpin=skySpinTarget=0;alt=targetAlt=homeAltitude();genesisProgress=options.motion?0:1;genesisStarted=performance.now();lastTime=genesisStarted;if(genesisProgress===1)awakening.settle(false);notifyAwakening();stillFrames=0;setStage('orbit');notifyTransformation(true);notifyGenesis(true);}
 ...
 function up(e:PointerEvent){lastInteraction=performance.now();pointer.delete(e.pointerId);pinch=0;if(host.hasPointerCapture(e.pointerId))host.releasePointerCapture(e.pointerId);void dragged;}
 function cancel(e:PointerEvent){lastInteraction=performance.now();pointer.delete(e.pointerId);pinch=0;}
 ...
 function frame(now:number){if(disposed||graphicsLost||document.hidden)return;if(!options.motion&&stillFrames>2&&!celestial.pending&&!activityTransitions.some(t=>t.pending)&&!heldKeys.size&&!flight&&!morph&&genesisProgress>=1&&personalSettledAt===null&&awakening.state!=='awakening'){raf=requestAnimationFrame(frame);return;}if(fallback&&now-lastTime<50){raf=requestAnimationFrame(frame);return;}const elapsed=Math.max(0,(now-lastTime)/1000),dt=Math.min(elapsed,.06);lastTime=now;awakening.advance(elapsed,options.motion);if(options.motion)time+=dt;layerClock+=dt;for(const transition of activityTransitions)transition.update(layerClock,reducedMotion);frameCount++;stillFrames++;updateHeld(dt);
 ...
 else{if(stage==='orbit'&&viewMode==='globe'&&!morph&&genesisProgress>=1&&openProgress===0&&!personalPlaces&&!targetId&&(!remembering||settledAt!==null&&now-settledAt>6500)&&options.motion&&options.autoRotate&&!compositionOpen&&!comparingLight&&pointer.size===0){const spin=dt*rotationRate(options,(now-lastInteraction)/1000,awakening.idleRate/.84);targetLon+=spin;skySpinTarget+=spin;}const smooth=options.motion?1-Math.exp(-dt*5):1;skySpin=THREE.MathUtils.lerp(skySpin,skySpinTarget,smooth);lat=THREE.MathUtils.lerp(lat,targetLat,smooth);lon=THREE.MathUtils.lerp(lon,targetLon,smooth);alt=THREE.MathUtils.lerp(alt,targetAlt,smooth);}
 if(morph){
 ...
 if(genesisProgress<1&&!genesisHeld){genesisProgress=options.motion?clamp((now-genesisStarted)/GENESIS_DURATION,0,1):1;if(genesisProgress===1)awakening.settle(options.motion);notifyGenesis(genesisProgress===1);}
 ...
 north.copy(geo(lon,lat+90));camera.position.copy(geo(lon,lat)).addScaledVector(geo(lon,lat),alt*Math.cos(tilt*R)).addScaledVector(north,-alt*Math.sin(tilt*R));camera.up.copy(north);camera.lookAt(geo(lon,lat).multiplyScalar(born.settled));const w=host.clientWidth,h=host.clientHeight,mobile=w<700;
 const available=compositionOpen&&compositionBounds?compositionFraming(w,h,compositionBounds,camera.position.length()):null;
 const planet=stage==='orbit'&&scaleTier==='planet'&&viewMode==='globe'&&openProgress===0&&!personalPlaces&&!selected?planetFraming(w,h,camera.position.length(),narrativeVisible?openingBounds:null):null;
 const offsetY=mobile?(selected?(storyInset?Math.max(0,(storyInset-70)/2):h*.225):openProgress>0?h*.25:personalPlaces&&stage==='orbit'?-h*.09:viewMode==='cutaway'?h*.11:stage==='orbit'&&scaleTier==='planet'?h*.015:-h*.035):0;
 const desktopInset=stage==='city'?(SPECIAL_CITIES[targetId??'']?.desktopOffset??.17):(narrativeVisible||openProgress>0||!!personalPlaces)?(w<1100?.09:.17):0;
 const frameMix=frameCount===1||!options.motion?1:1-Math.exp(-dt*6);
 smoothCameraOffset.lerp(new THREE.Vector2((available?.x??planet?.x??(mobile?(stage==='orbit'&&scaleTier==='planet'&&viewMode==='globe'&&openProgress===0?w*.035:0):-w*desktopInset))*born.settled,(available?.y??planet?.y??offsetY)*born.settled),frameMix);
 const panelZoom=available&&scaleTier==='planet'&&viewMode==='globe'?available.zoom:1+((planet?.zoom??1)-1)*born.settled;
 camera.zoom=THREE.MathUtils.lerp(camera.zoom,panelZoom,frameMix);
 camera.setViewOffset(w,h,smoothCameraOffset.x,smoothCameraOffset.y,w,h);
 if(available)host.dataset.compositionFrame=JSON.stringify({...available,zoom:camera.zoom});else delete host.dataset.compositionFrame;
 camera.near=Math.max(.0000004,alt*.002);camera.far=65;camera.updateProjectionMatrix();camera.updateMatrixWorld();
 ...
 coast.material.uniforms.shoreBreath.value=displayedLight.shoreBreath;
 ...
 updateSignals(time,born.settled*(1-opened),dt);updateSea(time,born.settled*(1-opened));updateWater(time,born.settled*(1-opened));updateUrban(time,levels.city*cityDim);
 ...
 for(const [field,intensity,own,other,colour] of [[populationField,displayedLight.population,popFocus,footFocus,'#769ebe'],[footprintField,displayedLight.footprint,footFocus,popFocus,'#9b79b4']] as const){if(!field)continue;
  field.material.uniforms.opacity.value=global*born.settled*(1-opened)*intensity*(1.15+own*6.7)*(1-other*.9)*(1-nightFocus*.9)*(1-connectionFocus*.75);
  field.material.uniforms.tint.value.set('#c2b7a5').lerp(new THREE.Color(colour),displayedLight.colour);
  field.material.uniforms.sizeScale.value=.96;
 ...
 genesisStarted=performance.now();lastTime=genesisStarted;if(genesisProgress===1)awakening.settle(options.motion);notifyAwakening();notifyGenesis(true);notifyTransformation(true);raf=requestAnimationFrame(frame);
 return {audioState,beginGenesis,replayGenesis,skipGenesis,command,worldState,transform:setTransformation,personal:setPersonal,descend:async()=>{if(flight||morph||genesisProgress<1||openProgress>0)return;try{personalSettledAt=null;personalArrivalSent=false;clearStory();remembered=null;remembering=false;settledAt=null;arrivalSent=false;activeOpenTarget=undefined;void openViews.setTarget(null);targetId='singapore';scaleTier='city';if(presentation.focus!=='living'){presentation={...presentation,focus:'living'};syncPresentation();}loadingDetail=true;fly(1.2965,103.851,.0018,'city');notifyWorld();try{await ensureCity();}finally{loadingDetail=false;stillFrames=0;notifyWorld();}if(disposed)return;}catch(e){if(!signal.aborted)callbacks.error('Singapore could not load. Your Earth is still here; try entering again.');throw e;}},orbit:()=>{if(flight||morph||stage!=='city')return;clearStory();remembering=!!remembered;settledAt=null;arrivalSent=false;activeOpenTarget=undefined;void openViews.setTarget(null);targetId=null;scaleTier='planet';fly(19,95,homeAltitude(),'orbit');notifyWorld();},zoom,rotate,select,view:setView,region:setRegion,configure:(o)=>{configuredLight={...DEFAULT_LIGHT,...o,transport:o.transport??cloneTransport()};if(!comparingLight)options={...configuredLight};syncTransport();syncPresentation();reducedMotion=matchMedia('(prefers-reduced-motion: reduce)').matches;for(const transition of activityTransitions)transition.update(layerClock,reducedMotion);if(!o.motion)awakening.advance(0,false);notifyAwakening();if(flight&&!o.motion)flight.duration=0;lastInteraction=performance.now();stillFrames=0;},storyInset:setStoryInset,compositionPanel:(open,bounds)=>{compositionOpen=open;compositionBounds=open?(bounds??compositionBounds):null;lastInteraction=performance.now();stillFrames=0;},narrativePanel:(visible,bounds)=>{narrativeVisible=visible;openingBounds=bounds??null;stillFrames=0;},compareLight:(light)=>{comparingLight=!!light;options={...(light??configuredLight),motion:configuredLight.motion};Object.assign(displayedLight,options);syncTransport();syncPresentation();lastInteraction=performance.now();stillFrames=0;},dispose:destroy};
}
