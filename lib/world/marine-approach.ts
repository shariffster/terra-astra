...
export function marineApproachReach(point: readonly [number, number]) {
  ...
  if(lat===33.875&&lon===142.125)return .135; // Japan offshore
  if(lat===11.875&&lon===145.125)return .120; // Guam offshore
  if(lat===21.875&&lon===-156.875)return .145; // Hawaii offshore
  if(lat===36.875&&lon===-124.875)return .110; // Northern California offshore
  if(lat===31.875&&lon===-119.875)return .095; // Southern California offshore
...
/** Smooth geographic weight for the Pacific display study. Outside it the
 * established Atlantic/Indian strand spacing and junction cuts stay exact. */
export function pacificFlowWeight(lat:number,lon:number) {
  const ease=(a:number,b:number,x:number)=>{const t=Math.max(0,Math.min(1,(x-a)/(b-a)));return t*t*(3-2*t);};
  const ocean=lon>=0?ease(125,145,lon):ease(112,132,-lon);
  return ocean*ease(-48,-36,lat)*(1-ease(48,60,lat));
}
