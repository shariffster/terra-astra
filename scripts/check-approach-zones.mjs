...
for(const surface of[true,false]){
 ...
 report.push({family:surface?'shipping':'cables',routes:paths.length,extendedRoutes:affected.length,reachDegrees:affected.flatMap(p=>p.pieces.filter(x=>x.approachReach).map(x=>x.approachReach*180/Math.PI)).reduce((a,x)=>[Math.min(a[0],x),Math.max(a[1],x)],[Infinity,0])});
}
