...
assert.deepEqual(awakeningOrder(shipSignals).slice(0,3).map(s=>s.id),['sea-south-china','sea-north-atlantic','sea-indian']);
...
const shells=()=>clouds().filter(o=>o.userData.shell);
...
tick(3000);assert.deepEqual(stats(),[72,49,0,0]);assert.ok(stats()[0]>first);
tick(2500);assert.ok(stats()[1]===120&&stats()[2]>0&&stats()[2]<24&&stats()[3]===0);
tick(2000);assert.ok(stats()[3]>0&&stats()[3]<10);tick(2000);assert.deepEqual(stats(),[72,120,24,10]);tick(3000);assert.equal(host.dataset.cablesFull,'10');tick(2000);assert.equal(host.dataset.awakening,'complete');assert.equal(host.dataset.awakeningSeconds,'18.000');assert.equal(sea.material.uniforms.seaMotion.value,1);
for(const shell of shells()){assert.equal(shell.geometry.drawRange.count,shell.geometry.getAttribute('position').count);const b=shell.geometry.getAttribute('brightness');for(let i=0;i<b.count;i+=9)assert.ok(Math.abs(b.getX(i)-1.65)<1e-6,'Full baseline head exposure');}
...
engine.configure({...opts,motion:false});tick();assert.equal(host.dataset.awakening,'complete');assert.deepEqual(stats(),[72,120,24,10]);for(const o of shells()){const b=o.geometry.getAttribute('brightness');for(let i=0;i<b.count;i++)if(i%9)assert.equal(b.getX(i),0);}
