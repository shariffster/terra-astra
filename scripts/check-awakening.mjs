...
for(const shell of shells()){assert.equal(shell.geometry.drawRange.count,shell.geometry.getAttribute('position').count);const b=shell.geometry.getAttribute('brightness'),stride=shell.userData.shell==='aircraft'?24:9,peak={satellites:1.65,aircraft:1.75,ships:1.95}[shell.userData.shell];let visible=0;for(let i=0;i<b.count;i+=stride){assert.ok(b.getX(i)>=0&&b.getX(i)<=peak+1e-6,'Family exposure remains bounded with composition density');if(b.getX(i)>0)visible++;}assert.ok(visible>b.count/stride*.65&&visible<b.count/stride,'Default density leaves breathing room while retaining the full identity pool');}
