...
const data=['marine-branches','indian-branches','regional-branches'].map(n=>JSON.parse(readFileSync(new URL('../public/data/networks/'+n+'.json',import.meta.url))));
...
for(const surface of [true,false]){
 ...
 assert.equal(source.length,surface?490:398,'No route count increase');
...
console.log(JSON.stringify({result:'PASS',accepted,waterVertices:count,distinctPacificBows:bows.length,minimumForward,sourceCounts:{sea:490,cables:398}}));
