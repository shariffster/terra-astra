...
const data=['marine-branches','indian-branches','regional-branches','european-branches','east-asian-branches','pacific-branches'].map(n=>JSON.parse(readFileSync(new URL('../public/data/networks/'+n+'.json',import.meta.url))));
...
for(const surface of [true,false]){
 ...
 assert.equal(source.length,surface?527:431,'Includes European and East Asian regional connections');
...
console.log(JSON.stringify({result:'PASS',accepted,waterVertices:count,distinctPacificBows:bows.length,minimumForward,sourceCounts:{sea:527,cables:431}}));
