async page => {
 ...
 const core=await page.evaluate(()=>window.terraAstra.getState().genesis);await page.screenshot({path:'output/playwright/qa-genesis-core.png'});
 ...
 const settlement=await page.locator('.universe').getAttribute('data-awakening-seconds');await page.screenshot({path:'output/playwright/qa-genesis-settlement.png'});
 await page.waitForFunction(()=>document.querySelector('.universe').dataset.awakening==='complete',{},{timeout:24000});
 const complete=await page.evaluate(()=>({...document.querySelector('.universe').dataset}));await page.screenshot({path:'output/playwright/qa-awakening-complete.png'});
 ...
 if(!lost)throw Error('Context loss could not be exercised');await page.waitForTimeout(500);await page.screenshot({path:'output/playwright/qa-context-loss.png'});
 const unavailable=await sound.count()?await sound.getAttribute('data-sound-status'):'control hidden after renderer loss';
 await page.emulateMedia({reducedMotion:'reduce'});await page.reload();await page.waitForFunction(()=>window.terraAstra&&!window.terraAstra.getState().busy);await page.screenshot({path:'output/playwright/qa-context-recovered.png'});
